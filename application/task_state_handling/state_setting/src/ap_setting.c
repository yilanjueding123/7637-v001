...
void ap_setting_init(INT32U prev_state, INT32U prev_state1, INT8U * tag)
{
	...
	if (prev_state == STATE_BROWSE)
	{
		...
		setting_browse_frame_buff = (INT32U)
		gp_malloc_align(getDispDevBufSize(), 64);
		...
		{
			msgQSend(StorageServiceQ, MSG_STORAGE_SERVICE_BROWSE_REQ, (void *) &search_type, sizeof(INT32U),
				 MSG_PRI_NORMAL);
		}
	}
	...
	else if ((prev_state == STATE_VIDEO_RECORD) || (prev_state == STATE_VIDEO_PREVIEW))
	{
		...
		{
			setting_browse_frame_buff = (INT32U)
			gp_malloc_align(TV_WIDTH * TV_HEIGHT * 2, 64);
			dest				= (INT8S *)
			setting_browse_frame_buff;
			Temp				= R_TV_FBI_ADDR;
			src 				= (INT8S *)
	...
	setting_category_set_fptr[0] = (SETTINGFUNC)
	ap_setting_video_record_set;
	setting_category_set_fptr[1] = (SETTINGFUNC)
	ap_setting_photo_capture_set;
	setting_category_set_fptr[2] = (SETTINGFUNC)
	ap_setting_browse_set;
	setting_category_set_fptr[3] = (SETTINGFUNC)
	ap_setting_basic_set;
...
INT32U ap_setting_show_GPZP_file(INT8U * path, INT16U * frame_buff, DISPLAY_ICONSHOW * icon_info, INT8U type)
{
	...
	if (logo_fd != 0xFFFF)
	{
		...
		icon_buffer 		= (INT32S)
		gp_malloc(icon_info->icon_w * icon_info->icon_h * 2);
...
void ap_setting_page_draw(INT32U state, INT32U state1, INT8U * tag)
{
	...
	ap_setting_show_GPZP_file((INT8U *) "BACKGROUND.GPZP", (INT16U *) setting_frame_buff, &icon,
		 SETTING_ICON_NORMAL_DRAW);
	...
	if (MODE_KEY_flag == 0)
	{
		...
		ap_setting_show_GPZP_file((INT8U *) "SELECTBAR_LONG.GPZP", (INT16U *) setting_frame_buff, &icon,
			 SETTING_ICON_NORMAL_DRAW);
...
void ap_setting_sub_menu_draw(INT8U curr_tag)
{
	...
	ap_setting_show_GPZP_file((INT8U *) "SELECTBOX_SUB.GPZP", (INT16U *) setting_frame_buff, &icon,
		 SETTING_ICON_NORMAL_DRAW);
	...
	{
		icon.pos_y			=
			 ICON_SELECTBOX_SUB_START_Y + (ICON_SELECTBOX_SUB_HEIGHT - ((ICON_SELECTBOX_SUB_HEIGHT / 4) * 2));
	}
	else 
	{
		icon.pos_y			=
			 ICON_SELECTBOX_SUB_START_Y + ((ICON_SELECTBOX_SUB_HEIGHT / 4 - ICON_SELECTBAR_MIDD_HEIGHT) >> 1);
	}

	i					= curr_tag & 0x3;
	icon.pos_y			= icon.pos_y + i * (ICON_SELECTBOX_SUB_HEIGHT / 4);
	ap_setting_show_GPZP_file((INT8U *) "SELECTBAR_MIDD.GPZP", (INT16U *) setting_frame_buff, &icon,
		 SETTING_ICON_NORMAL_DRAW);
	...
	{
		sub_str.pos_y		=
			 ICON_SELECTBOX_SUB_START_Y + (ICON_SELECTBOX_SUB_HEIGHT - ((ICON_SELECTBOX_SUB_HEIGHT / 4) * 2)) + ((ICON_SELECTBOX_SUB_HEIGHT / 4 - str_res.string_height) >> 1);
	}
	else 
	{
		sub_str.pos_y		=
			 ICON_SELECTBOX_SUB_START_Y + ((ICON_SELECTBOX_SUB_HEIGHT / 4 - str_res.string_height) >> 1);
	}
	...
	if ((setting_item.stage == STR_DEFAULT_SETTING) || (setting_item.stage == STR_FORMAT1))
	{
		...
		for (i = 0; i < 2; i++)
		{
			...
			ap_state_resource_string_resolution_get(&sub_str, &str_res);
			sub_str.pos_x		=
				 ICON_SELECTBOX_SUB_START_X + ((ICON_SELECTBOX_SUB_WIDTH - str_res.string_width) >> 1);
...
void ap_setting_date_time_menu_draw(INT8U tag)
{
	...
	ascii_str.pos_y 	=
		 ICON_SELECTBOX_SUB_START_Y + ((ICON_SELECTBOX_SUB_HEIGHT - (ICON_YEAR_SELECTBAR_HEIGHT * 3)) / 4) +icon_date_time_offset + ((ICON_YEAR_SELECTBAR_HEIGHT - String_height) >> 1);
	...
	ascii_str.pos_x 	= ICON_SELECTBOX_SUB_START_X + ((ICON_SELECTBOX_SUB_WIDTH - String_width) >> 1);
	ascii_str.pos_y 	=
		 ICON_SELECTBOX_SUB_START_Y + ICON_YEAR_SELECTBAR_HEIGHT + ((ICON_SELECTBOX_SUB_HEIGHT - (ICON_YEAR_SELECTBAR_HEIGHT * 3)) / 4) * 2 + ((ICON_YEAR_SELECTBAR_HEIGHT - String_height) >> 1);
	...
	ascii_str.pos_x 	= ICON_SELECTBOX_SUB_START_X + ((ICON_SELECTBOX_SUB_WIDTH - String_width) >> 1);
	ascii_str.pos_y 	=
		 ICON_SELECTBOX_SUB_START_Y + ICON_YEAR_SELECTBAR_HEIGHT * 2 + ((ICON_SELECTBOX_SUB_HEIGHT - (ICON_YEAR_SELECTBAR_HEIGHT * 3)) / 4) * 3 + ((ICON_YEAR_SELECTBAR_HEIGHT - String_height) >> 1) -icon_date_time_offset;
	...
	if (setup_date_time[6] == 0) //yy/mm/dd
	{
		if (tag == 0)
		{
			...
			Icon_pos_y			=
				 ICON_SELECTBOX_SUB_START_Y + ((ICON_SELECTBOX_SUB_HEIGHT - (ICON_YEAR_SELECTBAR_HEIGHT * 3)) / 4);
			...
			ap_setting_show_GPZP_file((INT8U *) "YEAR_SELECTBAR.GPZP", (INT16U *) setting_frame_buff, &icon,
				 SETTING_ICON_NORMAL_DRAW);
		...
		else 
		{
			...
			if (tag == 1)
			{
				...
				String_pos_x		=
					 ICON_SELECTBOX_SUB_START_X + ((ICON_SELECTBOX_SUB_WIDTH - String_width) >> 1) + (ASCII_draw_char_width * 7);
				Icon_pos_x			= String_pos_x - ((ICON_DAY_SELECTBAR_WIDTH - String_width1) >> 1);
				Icon_pos_y			=
					 ICON_SELECTBOX_SUB_START_Y + ((ICON_SELECTBOX_SUB_HEIGHT - (ICON_DAY_SELECTBAR_HEIGHT * 3)) / 4) +icon_date_time_offset;
			...
			else if (tag == 2)
			{
				...
				String_pos_x		=
					 ICON_SELECTBOX_SUB_START_X + ((ICON_SELECTBOX_SUB_WIDTH - String_width) >> 1) + (ASCII_draw_char_width * 12);
				Icon_pos_x			= String_pos_x - ((ICON_DAY_SELECTBAR_WIDTH - String_width1) >> 1);
				Icon_pos_y			=
					 ICON_SELECTBOX_SUB_START_Y + ((ICON_SELECTBOX_SUB_HEIGHT - (ICON_DAY_SELECTBAR_HEIGHT * 3)) / 4) +icon_date_time_offset;
			...
			else if (tag == 3)
			{
				...
				Icon_pos_x			= String_pos_x - ((ICON_DAY_SELECTBAR_WIDTH - String_width1) >> 1);
				Icon_pos_y			=
					 ICON_SELECTBOX_SUB_START_Y + (((ICON_SELECTBOX_SUB_HEIGHT - (ICON_DAY_SELECTBAR_HEIGHT * 3)) / 4) * 2) +ICON_DAY_SELECTBAR_HEIGHT;
			...
			else if (tag == 4)
			{
				...
				String_pos_x		=
					 ICON_SELECTBOX_SUB_START_X + ((ICON_SELECTBOX_SUB_WIDTH - String_width) >> 1) + (ASCII_draw_char_width * 5);
				Icon_pos_x			= String_pos_x - ((ICON_DAY_SELECTBAR_WIDTH - String_width1) >> 1);
				Icon_pos_y			=
					 ICON_SELECTBOX_SUB_START_Y + (((ICON_SELECTBOX_SUB_HEIGHT - (ICON_DAY_SELECTBAR_HEIGHT * 3)) / 4) * 2) +ICON_DAY_SELECTBAR_HEIGHT;
			...
			else if (tag == 5)
			{
				...
				String_pos_x		=
					 ICON_SELECTBOX_SUB_START_X + ((ICON_SELECTBOX_SUB_WIDTH - String_width) >> 1) + (ASCII_draw_char_width * 10);
				Icon_pos_x			= String_pos_x - ((ICON_DAY_SELECTBAR_WIDTH - String_width1) >> 1);
				Icon_pos_y			=
					 ICON_SELECTBOX_SUB_START_Y + (((ICON_SELECTBOX_SUB_HEIGHT - (ICON_DAY_SELECTBAR_HEIGHT * 3)) / 4) * 2) +ICON_DAY_SELECTBAR_HEIGHT;
			...
			else if (tag == 6)
			{
				...
				Icon_pos_x			= String_pos_x - ((ICON_DAY_SELECTBAR_WIDTH - String_width1) >> 1);
				Icon_pos_y			=
					 ICON_SELECTBOX_SUB_START_Y + (((ICON_SELECTBOX_SUB_HEIGHT - (ICON_DAY_SELECTBAR_HEIGHT * 3)) / 4) * 3) +ICON_DAY_SELECTBAR_HEIGHT * 2 - 3;
			...
			ap_setting_show_GPZP_file((INT8U *) "DAY_SELECTBAR.GPZP", (INT16U *) setting_frame_buff, &icon,
				 SETTING_ICON_NORMAL_DRAW);
...
void ap_setting_background_draw(INT32U state, INT32U state1, STRING_INFO * str, INT8U flag)
{
	...
	ap_setting_show_GPZP_file((INT8U *) "TOPBAR.GPZP", (INT16U *) setting_frame_buff, &icon,
		 SETTING_ICON_NORMAL_DRAW);
...
void ap_setting_general_state_draw(INT32U state, STRING_INFO * str, INT8U * tag)
{
	DISPLAY_ICONSHOW icon =
	{
		ICON_SETTING_HEAD_WIDTH, ICON_SETTING_HEAD_HEIGHT, TRANSPARENT_COLOR, 0, 0
	};
	...
	for (i = 0; i < draw_cnt; i++)
	{
		...
		{
			icon_setting_table	= (INT32U)
			setting_record_table[offset];
		}
		else if (state == STATE_VIDEO_PREVIEW)
		{
			icon_setting_table	= (INT32U)
			setting_capture_table[offset];
		}
		else if (state == STATE_BROWSE)
		{
			icon_setting_table	= (INT32U)
			setting_play_table[offset];
		}
		else if (state == STATE_SETTING)
		{
			icon_setting_table	= (INT32U)
			setting_base_table[offset];
		}
...
INT8U ap_setting_right_menu_active(STRING_INFO * str, INT8U type, INT8U * sub_tag)
{
	...
	switch (str->str_idx)
	{
		...
		case STR_EV:
			...
			{
				ap_setting_sensor_sccb_cmd_set((INT8U *) ap_setting_EV_data, str->font_color,
					 sizeof(ap_setting_EV_data) / setting_item.sub_item_max);
			}
			else 
			{
		...
		case STR_SHARPNESS:
			...
			else if (type == SUB_MENU_MOVE)
			{
				/*				if(str->font_color == 1){
									...
									ap_setting_sensor_command_switch(0xAC, 0x20, 1);
									ap_setting_sensor_sccb_cmd_set((INT8U *) ap_setting_Sharpness, str->font_color, 
					sizeof(ap_setting_Sharpness)/setting_item.sub_item_max);
								}
				*/
			}
			else 
		...
		case STR_WHITE_BALANCE:
			...
			else if (type == SUB_MENU_MOVE)
			{
				/*				if(str->font_color == 0){
								...
								ap_setting_sensor_sccb_cmd_set((INT8U *) ap_setting_White_Balance, str->font_color, 
					sizeof(ap_setting_White_Balance)/setting_item.sub_item_max);
				*/
			}
			else 
			{
		...
		/*		case STR_SCENE_MODE:
					...
					if (type == STRING_DRAW) {
						...
						ap_setting_sensor_sccb_cmd_set((INT8U *) ap_setting_Scene_Mode, str->font_color, sizeof(ap_
			setting_Scene_Mode)/setting_item.sub_item_max);
					} else if (type == FUNCTION_ACTIVE) {
		...
		case STR_ISO:
			...
			else if (type == SUB_MENU_MOVE)
			{
				/*
								...
								ap_setting_sensor_sccb_cmd_set((INT8U *) ap_setting_ISO_data, str->font_color, sizeof(
					ap_setting_ISO_data)/setting_item.sub_item_max);				
				*/
			}
			else 
			{
		...
		/*		case STR_COLOR:
					...
					if (type == STRING_DRAW) {
						...
						if(str->font_color == 0){
							...
							ap_setting_sensor_sccb_cmd_set((INT8U *) ap_setting_Color, str->font_color, sizeof(ap_
			setting_Color)/setting_item.sub_item_max);
						}
					} else {
				...
				case STR_SATURATION:
					...
					if (type == STRING_DRAW) {
						...
						ap_setting_sensor_sccb_cmd_set((INT8U *) ap_setting_Saturation, str->font_color, sizeof(ap_
			setting_Saturation)/setting_item.sub_item_max);
					} else {
		...
		*/
			...
			else if (type == FUNCTION_ACTIVE)
			{
				if (g_setting_delete_choice == 0)
				{
					if (str->font_color == 1)
					{
						...
						msgQSend(StorageServiceQ, MSG_STORAGE_SERVICE_VIDEO_FILE_DEL, &str->font_color,
							 sizeof(INT32U), MSG_PRI_NORMAL);
					}
				}
				else if (g_setting_delete_choice == 1)
				{
					if (str->font_color == 1)
					{
						...
						setting_item.stage	= 0xAA55; //wwj add
						msgQSend(StorageServiceQ, MSG_STORAGE_SERVICE_FILE_DEL_ALL, &str->font_color, sizeof(INT32U),
							 MSG_PRI_NORMAL);
						OSTimeDly(50);
					}
				}
		...
		case STR_PROTECT:
			...
			else if (type == FUNCTION_ACTIVE)
			{
				if (g_setting_lock_choice == 0)
				{
					...
					{
						setting_item.stage	= 0xAA55; //wwj add
						msgQSend(StorageServiceQ, MSG_STORAGE_SERVICE_LOCK_ONE, &str->font_color, sizeof(INT32U),
							 MSG_PRI_NORMAL);
					}
				}
				else if (g_setting_lock_choice == 1)
				{
					...
					{
						setting_item.stage	= 0xAA55; //wwj add
						msgQSend(StorageServiceQ, MSG_STORAGE_SERVICE_UNLOCK_ONE, &str->font_color, sizeof(INT32U),
							 MSG_PRI_NORMAL);
					}
				}
				else if (g_setting_lock_choice == 2)
				{
					if (str->font_color == 1)
					{
						ap_setting_busy_show();
						setting_item.stage	= 0xAA55; //wwj add
						msgQSend(StorageServiceQ, MSG_STORAGE_SERVICE_LOCK_ALL, &str->font_color, sizeof(INT32U),
							 MSG_PRI_NORMAL);
						OSTimeDly(50);
					}
				}
				else if (g_setting_lock_choice == 3)
				{
					if (str->font_color == 1)
					{
						ap_setting_busy_show();
						setting_item.stage	= 0xAA55; //wwj add
						msgQSend(StorageServiceQ, MSG_STORAGE_SERVICE_UNLOCK_ALL, &str->font_color, sizeof(INT32U),
							 MSG_PRI_NORMAL);
						OSTimeDly(50);
					}
				}
		...
		/*		case STR_DELETE_ONE:
					if (type == STRING_DRAW) {
						...
						if (str->font_color == 1) {
							msgQSend(StorageServiceQ, MSG_STORAGE_SERVICE_VIDEO_FILE_DEL, &str->font_color, sizeof(
			INT32U), MSG_PRI_NORMAL);
						}
					}
					break;
				case STR_DELETE_ALL:
					if (type == STRING_DRAW) {
						...
						if (str->font_color == 1) {
							msgQSend(StorageServiceQ, MSG_STORAGE_SERVICE_FILE_DEL_ALL, &str->font_color, sizeof(
			INT32U), MSG_PRI_NORMAL);
						}
					}
					break;
...

				{
					*sub_tag			+= 1;
					...
					if (setup_date_time[6] == 0)
					{
						...
						{
							setting_item.sub_item_start = 1;
							setting_item.sub_item_max = CalendarCalculateDays(setup_date_time[1],
								 setup_date_time[0]) + 1;
						}
						else if (*sub_tag == 3)
						{
...
void ap_setting_date_time_string_process(char * dt_str1, INT8U dt_tag)
{
	...
	{ //DD/MM/YY
		...
		INT8U			tag[] =
		{
			0, 1, 5, 6, 12, 13, 17, 18, 22, 23, 27, 28
		};
...
INT8U Get_cmputer_build_time(INT8U * buf)
{
	...
	INT8U			Months[12][6] =
	{
		"Jan01", "Feb02", "Mar03", "Apr04", "May05", "Jun06", "Jul07", "Aug08", "Sep09", "Oct10", "Nov11", "Dec12"
	};
