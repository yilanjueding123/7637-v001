	...
	if (type == TRUE)
	{
		if (storage_freesize_timerid == 0xFF)
		{
			...
			sys_set_timer((void *) msgQSend, (void *) StorageServiceQ, MSG_STORAGE_SERVICE_FREESIZE_CHECK,
				 storage_freesize_timerid, STORAGE_TIME_INTERVAL_FREESIZE); //STORAGE_TIME_INTERVAL_FREESIZE
...
void ap_storage_service_file_open_handle(INT32U req_type)
{
	...
	switch (req_type)
	{
		case MSG_STORAGE_SERVICE_VID_REQ:
			...
			while ((avi_file_table[g_avi_index >> 4] & (1 << (g_avi_index & 0xF))))		//检测table表中对应文件索引号存储的位置文件信息
			{
				stat(g_file_path, &buf_tmp);
