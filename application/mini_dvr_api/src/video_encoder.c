...
CODEC_START_STATUS video_encode_start(MEDIA_SOURCE src, INT16S txt_handle)
{
	...
	nRet				= avi_encode_set_file_handle_and_caculate_free_size(pAviEncPara->AviPackerCur,
		 src.type_ID.FileHandle, txt_handle);
...
CODEC_START_STATUS video_encode_fast_stop_and_start(MEDIA_SOURCE src, INT16S next_txt_handle)
{
	...
	nRet				= avi_enc_packer_switch_file(pAviEncPara->AviPackerCur, src.type_ID.FileHandle,
		 next_txt_handle);
