...
void cdsp_overflow_isr_func(void)
{
	...
	if (PreviewIsStopped)
	{
		...
		else if (cdsp_overflow_count > 10 * 30)
		{ //10s
			cdsp_hangup_flag	= 1;
		}
		else 
		...
		*/
		...
		{ //error occur
			...
			if (((fifo_addr_bak0 >= 0xF8000000) && (fifo_addr_bak0 < 0xF8040000)) ||
				 ((fifo_addr_bak0 >= USE_SDRAM_SUPPORT_FIFO_ADDRS_START) &&
				 (fifo_addr_bak0 < USE_SDRAM_SUPPORT_FIFO_ADDRS_END)))
#endif
...
void zoom_in_out_func(void)
{
	...
	if ((gZoomArgs.source_width == SENSOR_WIDTH) || (gZoomArgs.source_width == 960))
	{
...
INT32S video_preview_start(preview_args_t * pPreviewArgs)
{
	...
	sensor_frame_range_t sensorFrameRange =
	{
		0
	};
	OS_CPU_SR cpu_sr;
	...
	*/
	...
	if ((gGpreviewArgs.sensor_width == SENSOR_WIDTH) && (gGpreviewArgs.sensor_height == SENSOR_HEIGHT))
	{
	...
	else if ((gGpreviewArgs.sensor_width == 960) && (gGpreviewArgs.sensor_height == 720))
	{
		sensorFrameRange.point_x = (SENSOR_WIDTH - clip_width) >> 1;
		sensorFrameRange.point_y = (SENSOR_HEIGHT - clip_height) >> 1;
	...
	else if ((gGpreviewArgs.sensor_width == 848) && (gGpreviewArgs.sensor_height == 480))
	{
	...
	else if ((gGpreviewArgs.sensor_width == 640) && (gGpreviewArgs.sensor_height == 480))
	{
	...
	else if ((gGpreviewArgs.sensor_width == 320) && (gGpreviewArgs.sensor_height == 240))
	{
		sensorFrameRange.point_x = (SENSOR_WIDTH - 960) >> 1;
		sensorFrameRange.point_y = (SENSOR_HEIGHT - 720) >> 1;
		sensorFrameRange.clip_w = 960;
...
INT32S video_preview_zoom_in_out(INT8U zoomInOut, INT8U zoomIn_zoomOut)
{
	...
	else if ((gGpreviewArgs.sensor_width == 960) && (gGpreviewArgs.sensor_height == 720))
	{
		...
		ZOOM_HEIGHT_INTERVAL = 72;
		gZoomArgs.sensor_clip_range.point_x =
			 ((SENSOR_WIDTH - 960) >> 1) + ((ZOOM_WIDTH_INTERVAL * gZoomArgs.zoom_step) >> 1);
		gZoomArgs.sensor_clip_range.point_y = ((ZOOM_HEIGHT_INTERVAL * gZoomArgs.zoom_step) >> 1);
	...
	else if ((gGpreviewArgs.sensor_width == 848) && (gGpreviewArgs.sensor_height == 480))
	{
		...
		gZoomArgs.sensor_clip_range.point_x =
			 ((SENSOR_WIDTH - 1268) >> 1) + ((ZOOM_WIDTH_INTERVAL * gZoomArgs.zoom_step) >> 1);
		gZoomArgs.sensor_clip_range.point_y = (ZOOM_HEIGHT_INTERVAL * gZoomArgs.zoom_step) >> 1;
...
void cpu_draw_time_osd(TIME_T current_time, INT32U target_buffer, INT8U draw_type, INT8U state)
{
		...
		if ((state & 0xF) == (STATE_VIDEO_RECORD & 0xF))
		{
			if (FIFO_LINE_LN == 16)
			{
				...
				{ // The last part of date stamp
					str_info.pos_y		= 0;
					str_info.font_offset_h = 24;
				}
				else if (state & 0x40)
				{ // The 2nd part of date stamp 
					str_info.pos_y		= 0;
					str_info.font_offset_h = 8; 	//字的第几行
				}
				else 
			...
			else 
			{
				...
				{ // The 2nd part of date stamp 
					str_info.pos_y		= 0;
					str_info.font_offset_h = 8; 	// 20;
				}
				else 
				{ // The first part of date stamp
					str_info.pos_y		= 24;		//12;
	...
	switch (year_mon_day_format)
	{
		case 0: // Y/M/D
		default:
			tm1 = current_time.tm_year / 1000;		//kilobit
			ap_state_resource_char_draw_osd(tm1 + 0x30, (INT16U *) target_buffer, &str_info, draw_type, (state ==
				 (STATE_AUDIO_RECORD & 0xF)) ? 2: 1);
			current_time.tm_year -= tm1 * 1000;
			str_info.pos_x += gap_1;
			tm1 = current_time.tm_year / 100;		//hundreds
			ap_state_resource_char_draw_osd(tm1 + 0x30, (INT16U *) target_buffer, &str_info, draw_type, (state ==
				 (STATE_AUDIO_RECORD & 0xF)) ? 2: 1);
			current_time.tm_year -= tm1 * 100;
			str_info.pos_x += gap_1;
			tm1 = current_time.tm_year / 10;		//decade
			ap_state_resource_char_draw_osd(tm1 + 0x30, (INT16U *) target_buffer, &str_info, draw_type, (state ==
				 (STATE_AUDIO_RECORD & 0xF)) ? 2: 1);
			current_time.tm_year -= tm1 * 10;		//a bit
			str_info.pos_x += gap_1;
			ap_state_resource_char_draw_osd(current_time.tm_year + 0x30, (INT16U *) target_buffer, &str_info, draw_type, (state ==
				 (STATE_AUDIO_RECORD & 0xF)) ? 2: 1);
			str_info.pos_x += gap_1;				// '/'
			ap_state_resource_char_draw_osd(0x2F, (INT16U *) target_buffer, &str_info, draw_type, (state ==
				 (STATE_AUDIO_RECORD & 0xF)) ? 2: 1); 
			str_info.pos_x += gap_1;
			tm1 = current_time.tm_mon / 10;			// decade month
			ap_state_resource_char_draw_osd(tm1 + 0x30, (INT16U *) target_buffer, &str_info, draw_type, (state ==
				 (STATE_AUDIO_RECORD & 0xF)) ? 2: 1);
			current_time.tm_mon -= tm1 * 10;		// a bit month
			str_info.pos_x += gap_1;
			ap_state_resource_char_draw_osd(current_time.tm_mon + 0x30, (INT16U *) target_buffer, &str_info, draw_type, (state ==
				 (STATE_AUDIO_RECORD & 0xF)) ? 2: 1);
			str_info.pos_x += gap_1;
			ap_state_resource_char_draw_osd(0x2F, (INT16U *) target_buffer, &str_info, draw_type, (state ==
				 (STATE_AUDIO_RECORD & 0xF)) ? 2: 1); //  "/"
			str_info.pos_x += gap_1;
			tm1 = current_time.tm_mday / 10;		// decade day
			ap_state_resource_char_draw_osd(tm1 + 0x30, (INT16U *) target_buffer, &str_info, draw_type, (state ==
				 (STATE_AUDIO_RECORD & 0xF)) ? 2: 1);
			current_time.tm_mday -= tm1 * 10;		// a bit day
			str_info.pos_x += gap_1;
			ap_state_resource_char_draw_osd(current_time.tm_mday + 0x30, (INT16U *) target_buffer, &str_info, draw_type, (state ==
				 (STATE_AUDIO_RECORD & 0xF)) ? 2: 1);
	...
	if (enable_draw_time_flag == DRAW_DATE_TIME)
	{
		...
		ap_state_resource_char_draw_osd(current_time.tm_hour + 0x30, (INT16U *) target_buffer, &str_info, draw_type, (state ==
			 (STATE_AUDIO_RECORD & 0xF)) ? 2: 1);
		str_info.pos_x		+= gap_1;

		ap_state_resource_char_draw_osd(0x3A, (INT16U *) target_buffer, &str_info, draw_type, (state ==
			 (STATE_AUDIO_RECORD & 0xF)) ? 2: 1);	//	":"
		str_info.pos_x		+= gap_1;
		...
		ap_state_resource_char_draw_osd(current_time.tm_min + 0x30, (INT16U *) target_buffer, &str_info, draw_type, (state ==
			 (STATE_AUDIO_RECORD & 0xF)) ? 2: 1);
		str_info.pos_x		+= gap_1;
		...
		ap_state_resource_char_draw_osd(current_time.tm_sec + 0x30, (INT16U *) target_buffer, &str_info, draw_type, (state ==
			 (STATE_AUDIO_RECORD & 0xF)) ? 2: 1);
	}
