...
static INT32S usb_uac_task_create(void)
{
	...
	err 				= OSTaskCreate(uac_entry, (void *) NULL, &USBAudioStack[C_USB_AUDIO_STACK_SIZE - 1], 
		USB_AUDIO_PRIORITY);
...
static INT32S usb_uvc_task_create(void)
{
	...
	//creat usb cam task
	err 				= OSTaskCreate(uvc_entry, (void *) NULL, &USBCamStateStack[C_USB_CAM_STATE_STACK_SIZE - 1], 
		USB_CAM_PRIORITY);
