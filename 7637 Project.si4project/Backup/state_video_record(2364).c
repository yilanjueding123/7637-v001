...
extern volatile INT8U MODECTION_FLAG;
...
void state_video_record_entry(void * para, INT32U state)
{
	...
	power_on_flag		= 0;
	DBG_PRINT("state_video_record_entry\n");
	...
	while (exit_flag == EXIT_RESUME)
	{
		...
		switch (msg_id)
		{
			...
			case MSG_APQ_MOTION_DETECT_ACTIVE:
				DBG_PRINT("MSG_APQ_MOTION_DETECT_ACTIVE\n");
			...
			case MSG_APQ_POWER_ON_AUTO_RECORD:
				DBG_PRINT("video_status = %d, power_on_flag = %d\n", ap_video_record_sts_get(), power_on_flag);
				...
				if (power_on_flag /*|| card_insert_autorecord*/)
				{
					...
					DBG_PRINT("AUTO_Record_start status begining\n");
				}
