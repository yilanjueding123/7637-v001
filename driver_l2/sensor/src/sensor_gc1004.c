...
gpCdspWBGain_t * gc1004_awb_r_b_gain_boundary(void)
{
	...
	for (i = 10; i < 55; i++)
	{
		if (max_r_gain < g_GC1004_r_b_gain[i][0])
			max_r_gain = g_GC1004_r_b_gain[i][0];

		if (max_b_gain < g_GC1004_r_b_gain[i][1])
			max_b_gain = g_GC1004_r_b_gain[i][1];

		if (min_r_gain > g_GC1004_r_b_gain[i][0])
			min_r_gain = g_GC1004_r_b_gain[i][0];

		if (min_b_gain > g_GC1004_r_b_gain[i][1])
			min_b_gain = g_GC1004_r_b_gain[i][1];
	}
...
static void gc1004_cvt_analog_gain(INT32U analog_gain)
{
	...
	INT32U Analog_Multiple[11] =
	{
		1000, 1420, 1990, 2850, 4030, 5770, 8060, 11530, 16120, 23300, 32750
	};
...
int gc1004_set_exposure_time(sensor_exposure_t * si)
{
	...
	if (gc1004_seInfo.sensor_ev_idx >= gc1004_seInfo.max_ev_idx)
		gc1004_seInfo.sensor_ev_idx = gc1004_seInfo.max_ev_idx;

	if (gc1004_seInfo.sensor_ev_idx < 0)
		gc1004_seInfo.sensor_ev_idx = 0;
